...
public class AdminMemberController extends HttpServlet {
	...
	private void leaveMember(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		
		String userId =  request.getParameter("userId");
		Member member = memberService.selectMemberById(userId);
		
		System.out.println(userId);
		if(MemberGrade.valueOf(member.getGrade()).ROLE.equals("admin")) {
			request.setAttribute("msg", "관리자는 탈퇴시킬수 없습니다.");
			request.setAttribute("url", "/admin/member/member-list");
			request.getRequestDispatcher("/common/result").forward(request, response);
			return;
		}
		memberService.deleteMember(userId);
		request.setAttribute("msg", "회원이 정상적으로 탈퇴 되었습니다");
